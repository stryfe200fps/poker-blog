<?php

use App\Models\Article;
use App\Models\ArticleCategory;
use Spatie\Newsletter\NewsletterListCollection;
use Spatie\ImageOptimizer\OptimizerChainFactory;
use Spatie\LaravelImageOptimizer\Facades\ImageOptimizer;

it('optimizes the image', function () {
    superAdminAuthenticate();

    $this->get('admin/article/create')->assertStatus(200);

    $image = "data:image/png;base64,";

    // dd(base64_encode(file_get_contents('https://www.fillmurray.com/640/360')));
    $data = Article::factory()->make([
        'title' => 'Things I do',
        'article_categories' => collect(ArticleCategory::factory()->times(2)->create())->pluck('id')->toArray(),
    ])->attributesToArray();

    // dd(array_merge($data, [ 'image' => $image ]) );

    $datas = $this->post('admin/article', array_merge($data, [ 'image' => $image ]));

    $article = Article::find(1);
    $media = $article->media()->get()[0]->getPath();

    dd($media);

    ImageOptimizer::optimize($media);
    $size = filesize($media);

    #todoadi

    // $img = app(Spatie\ImageOptimizer\OptimizerChain::class)->optimize($media);
    $newMedia = $article->media()->get()[0]->getPath();
    // $newLocation = preg_replace('/.tmp/', '.jpg', $newMedia);
    // rename($newMedia, $newLocation);
    // move_uploaded_file($newMedia, $newLocation );

    // dump('before: '. filesize($newLocation));


    // dump('after: '. filesize($newLocation));


    // dd($size, $media, filesize($newLocation));


    $this->assertDatabaseHas('articles', ['title' => 'Things I do',
    ]);

    Article::find(1)->delete();


});