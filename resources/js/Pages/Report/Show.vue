        ...
        <div class="block-content">
            ...
            <div class="single-post-box">
                <div class="day-divider" style="border-bottom: 1px solid #d3d3d3; margin-top: 20px;">
                    <span>Day: {{report.data.day}}</span><br />
                    <span>{{report.data.level.name}}</span><br />
                </div>
                <div class="title-post">
                    ...
                    <div style="display:flex; justify-content: space-between;">
                        <div>
                            <ul class="post-tags">
                                <li><i class="fa fa-clock-o"></i>{{moment(report.data.date_added).format("MMM D YYYY")}}</li>
                                <li><i class="fa fa-user"></i>by <a href="#">{{report.data.article_author.first_name}} {{report.data.article_author.last_name}}
                                        </a>
                                </li>
                            </ul>
                        </div>
                ...
                <div :class="report.data.main_image ? 'post-content-min-height' : '' "  class="post-content">
                    <div class="post-gallery float-img" v-if="report.data.main_image"
                        style="float: left; margin: 0px 15px 5px 0px;">
                        <div style="position: relative;">
                            <img :src="report.data.main_image" alt="" style="margin-bottom: unset;" :style="[report.data.theme ? { 'filter': 'brightness(0.8)' } : {}]">
                            <div class="imageFrame" :style="{ 'background-image': 'url(' + getFrame(report.data.theme) + ')' }"></div>
                        </div>
                        <span v-if="report.data.caption" class="image-caption">{{report.data.caption}}</span>
                    </div>
                ...
                <div v-if="report.data.event_chips" style="margin-bottom: 20px;">
                    <CustomeTable>
                        <template v-slot:table-body>
                            <tr v-for="(item, index) in report.data.event_chips" :key="index">
                                <td>
                                ...
                                <td class="text-center hide-on-tablet" v-if="item.player.country"><CountryFlag :title="item.player.country.name" :iso="item.player.country.iso_3166_2" /></td>
                                <td class="text-center hide-on-tablet" v-else >?</td>
                                <td class="text-right">{{ item.current_chips === 0 ? 'BUSTED': item.current_chips.toLocaleString()}}</td>
                                <td class="text-right hide-on-mobile" >{{ item.current_chips === 0 ? '':item.changes.toLocaleString() }} 
                ...
                <div class="post-tags-box">
                    <ul class="tags-box">
                        <li><i class="fa fa-tags"></i><span>Tags:</span><a href="#" v-for="tag in report.data.event_chips" :key="tag.id">{{tag.player.name}}</a></li>
                    </ul>
                </div>
            </div>
        </div>
